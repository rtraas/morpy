...
def doPlot(sim, mirrorOrbit, astroInputs, rebInputs, simResults, Energy, times, file_dir,
           plotOutput, plotTypes, totalEnergyREB, infile, openlogfile=None): #TODO Delete totalEnergyREB here and in energies
...
# Deal with the matplotlib setup
    ...
    if plotOutput == 1: 
       matplotlib.use('Agg')
       print("Saving Fig Files",file=openlogfile)
    if plotOutput != 1 and plotOutput != 4: 
       if  os.environ.get('DISPLAY','') == '':
           print('No display found. Using non-interactive Agg backend.',file=openlogfile)
           matplotlib.use('Agg')
           if plotOutput == 2: print("No plots will be displayed or output",file=openlogfile)
           if plotOutput == 3: print("Will save figs but not plot to screen",file=openlogfile)
       else:
           matplotlib.use('TkAgg')
           if plotOutput == 2: print("Printing Fig Files to Screen Only",file=openlogfile)
           if plotOutput == 3: print("Saving Fig Files & Plotting to Screen",file=openlogfile)
