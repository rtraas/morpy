...
def setUpSim(mirrorOrbit, astroInputs, rebInputs, simResults, energy, verbose:bool=True,openlogfile=None):
    import rebound
    import math
    try:
    ...
    convertUnits(sim, mirrorOrbit, astroInputs, rebInputs,openlogfile=openlogfile)
    ...
    if rebInputs.addUsingOrbitalElements == True:
        if verbose:
            print("Orbital Elements Input",file=openlogfile)
    else:
        if verbose:
            print("Cartesian Input",file=openlogfile)
    
    # Now that everything is set up and the units are consistent units, add them
    # to the simulation.
    addParticles(sim, mirrorOrbit, astroInputs, rebInputs, openlogfile=openlogfile)
    ...
    if verbose:
        print("Mirror orbit time: ", simResults.torbMirror,file=openlogfile)
    ...
    if rebInputs.outputMegno:
        ...
        print ('\n'.join([str(mp) for mp in p[sim.N_real:sim.N_var+sim.N_var]]),file=openlogfile)
    ...
    if rebInputs.addForce != None: # If there are additional units specified, add them
        setUpAdditional(sim, astroInputs, rebInputs, mirrorOrbit, simResults,verbose=verbose,openlogfile=openlogfile)
